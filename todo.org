...
*** Implement due-predicates in model
    unit tests + model methods
    update fixtures to make some overdue, some due, some not due, some done
*** Indicate not-done/approaching-deadline states visually in-app
